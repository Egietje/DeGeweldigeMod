...
public class MapGenCheeseVillage extends MapGenStructure {
	public static List<Biome> VILLAGE_SPAWN_BIOMES = Arrays
			.<Biome>asList(new Biome[] { CheeseBiomes.CHEESE_BIOME });
	/** None */
	...
	public StructureStart getStructureStart(int chunkX, int chunkZ) {
		return new MapGenCheeseVillage.Start(this.worldObj, this.rand, chunkX, chunkZ, this.size);
	}
	...
	public static class Start extends StructureStart {
		...
		public Start(World worldIn, Random rand, int x, int z, int size) {
			...
			StructureVillagePieces.Start StructureCheeseVillagePieces$start = new StructureVillagePieces.Start(
					worldIn.getBiomeProvider(), 0, rand, (x << 4) + 2, (z << 4) + 2, list, size);
			this.components.add(StructureCheeseVillagePieces$start);
			StructureCheeseVillagePieces$start.buildComponent(StructureCheeseVillagePieces$start, this.components,
					rand);
			List<StructureComponent> list1 = StructureCheeseVillagePieces$start.pendingRoads;
			List<StructureComponent> list2 = StructureCheeseVillagePieces$start.pendingHouses;

			while (!list1.isEmpty() || !list2.isEmpty()) {
				if (list1.isEmpty()) {
					int i = rand.nextInt(list2.size());
					StructureComponent structurecomponent = (StructureComponent) list2.remove(i);
					structurecomponent.buildComponent(StructureCheeseVillagePieces$start, this.components, rand);
				} else {
					int j = rand.nextInt(list1.size());
					StructureComponent structurecomponent2 = (StructureComponent) list1.remove(j);
					structurecomponent2.buildComponent(StructureCheeseVillagePieces$start, this.components, rand);
				}
			}
